...
wss.on("connection", function connection(ws, request) {
  ...
  ws.send(
    JSON.stringify({
      type: "welcome",
      message:
        "Chào mừng bạn đến với WebSocket Server! Vui lòng gửi tin nhắn register để đăng ký.",
  ...
  ws.on("message", function message(data) {
    try {
      ...
      console.log(
        `Client hiện tại - Role: ${ws.role}, ID: ${ws.clientId}, Type: ${parsedData.type}`
      );
      ...
      switch (parsedData.type) {
        ...
        case "new_post":
          ...
          if (
            postTargetClient &&
            postTargetClient.readyState === WebSocket.OPEN
          ) {
            const postData = {
              type: "new_post",
            ...
            console.log(
              `Post từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
        ...
        case "comment":
          ...
          if (
            commentTargetClient &&
            commentTargetClient.readyState === WebSocket.OPEN
          ) {
            const commentData = {
              type: "comment",
            ...
            console.log(
              `Comment từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
        ...
        case "comment_byB":
          ...
          if (
            commentTargetCRM &&
            commentTargetCRM.readyState === WebSocket.OPEN
          ) {
            const commentData = {
              type: "comment_byB",
              postId: parsedData.postId,
              ...
              URL: parsedData.URL || "",
              commentFbId:
                parsedData.commentFbId || parsedData.id_facebookComment || "1",
              timestamp: new Date().toISOString(),
              from: ws.clientId,
            };
        ...
        case "URL_post":
          ...
          if (
            urlTargetClient &&
            urlTargetClient.readyState === WebSocket.OPEN
          ) {
            const URLData = {
              type: "URL_post",
            ...
            console.log(
              `URL từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
        ...
        case "reply_comment":
          ...
          if (
            replyTargetClient &&
            replyTargetClient.readyState === WebSocket.OPEN
          ) {
            ...
            console.log(
              `reply_comment từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
          ...
          if (replyTargetCRM && replyTargetCRM.readyState === WebSocket.OPEN) {
            ...
            ws.send(
              JSON.stringify({
                type: "error",
                message:
                  'Vui lòng sử dụng type "reply_comment" với trường "to" để chỉ định client đích',
                timestamp: new Date().toISOString(),
              })
            );
          }
        ...
        case "reply_reply_comment":
          ...
          if (
            replyReplyTargetClient &&
            replyReplyTargetClient.readyState === WebSocket.OPEN
          ) {
            ...
            console.log(
              `Đã gửi reply_reply_comment đến client ${parsedData.to}`
            );
            ...
            console.log(
              `reply_reply_comment từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
        ...
        case "comment_result":
          ...
          if (
            commentResultTargetClient &&
            commentResultTargetClient.readyState === WebSocket.OPEN
          ) {
            ...
            console.log(
              `comment_result từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
        ...
        case "reply_comment_result":
          ...
          if (
            replyCommentResultTargetClient &&
            replyCommentResultTargetClient.readyState === WebSocket.OPEN
          ) {
            ...
            replyCommentResultTargetClient.send(
              JSON.stringify(replyCommentResultData)
            );
            console.log(
              `Đã gửi reply_comment_result đến client ${parsedData.to}`
            );
            ...
            console.log(
              `reply_comment_result từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            console.log(
              `Client ${ws.clientId} với role ${ws.role} không được phép gửi reply_comment_result`
            );
            ws.send(
              JSON.stringify({
                type: "error",
        ...
        case "reply_reply_comment_result":
          ...
          console.log(
            `Xử lý reply_reply_comment_result từ client: ${ws.clientId}`
          );
          console.log(
            "Dữ liệu reply_reply_comment_result nhận được:",
            parsedData
          );
          if (!parsedData.to) {
            ws.send(
          ...
          const replyReplyCommentResultTargetClient = clients.get(
            parsedData.to
          );
          if (
            replyReplyCommentResultTargetClient &&
            replyReplyCommentResultTargetClient.readyState === WebSocket.OPEN
          ) {
            ...
            console.log(
              `Dữ liệu ${parsedData.type} sẽ gửi:`,
              replyReplyCommentResultData
            );

            replyReplyCommentResultTargetClient.send(
              JSON.stringify(replyReplyCommentResultData)
            );
            console.log(
              `Đã gửi ${parsedData.type} đến client ${parsedData.to}`
            );
            ...
            console.log(
              `${parsedData.type} từ ${ws.clientId} đã được gửi đến ${parsedData.to}`
            );
          } else {
            ws.send(
              JSON.stringify({
        ...
        case "crawl_comment":
          ...
          if (
            crawlCommentTargetClient &&
            crawlCommentTargetClient.readyState === WebSocket.OPEN
          ) {
            ...
            console.log(
              `Dữ liệu ${parsedData.type} sẽ gửi:`,
              crawlCommentResultData
            );

            crawlCommentTargetClient.send(
              JSON.stringify(crawlCommentResultData)
            );
            console.log(
              `Đã gửi ${parsedData.type} đến client ${parsedData.to}`
            );
        ...
        case "crawl_comment_by_CRM":
          ...
          if (
            crawlCommentCRMTargetClient &&
            crawlCommentCRMTargetClient.readyState === WebSocket.OPEN
          ) {
            const crawlCommentResultData = {
              type: "crawl_comment_by_CRM",
              status: parsedData.status,
              message: parsedData.message,
              postId: parsedData.postCount,
              URL: parsedData.currentPost,
              authorId: parsedData.authorId,
              from: ws.clientId,
              facebookId: parsedData.facebookId,
            };
            ...
            crawlCommentCRMTargetClient.send(
              JSON.stringify(crawlCommentResultData)
            );
            console.log(
              `Đã gửi ${parsedData.type} đến client ${parsedData.to}`
